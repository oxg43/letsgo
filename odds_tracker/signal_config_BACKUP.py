...
#
# UPDATED 2026-02-18: Based on professional analysis of 81 paper trades
# Key findings:
#   - Away bets losing -17.5% ROI → tighten to drop >= 15%
#   - Odds 1.30-1.50 losing -29% edge → add banned_odds_ranges
#   - Drop 6-10% losing -34.5% ROI → add banned_drop_ranges
#   - Confidence <80% losing → enforce min_confidence
#
ANTI_FILTERS = {
    '1': {  # Domaćin - RADI DOBRO (+5.9% ROI)
        'max_closing_odds': 3.00,    # Autsajderi: ROI = -35.3%
        'min_drop_pct': 5.0,         # Ispod 5% nema dovoljno signala
        'banned_odds_ranges': [(1.30, 1.50)],  # -29% edge u ovom rasponu
        'banned_drop_ranges': [(6.0, 10.0)],   # Middle drops gube -34.5%
    },
    'X': {  # Remi - RADI OK
        'max_closing_odds': 3.50,    # Visoke kvote remija: ROI = -31.6%
        'min_drop_pct': 10.0,        # Ispod 10%: EV negativan
    },
    '2': {  # Gost - GUBI (-17.5% ROI) → STRIKTNIJE!
        'max_closing_odds': 2.00,    # Samo jaki favoriti (smanjeno s 3.00)
        'min_drop_pct': 15.0,        # POVEĆANO s 10% na 15%
        'banned_drop_ranges': [(6.0, 10.0)],   # Middle drops gube
    },
    'global': {
        'max_minutes_to_ko': 30,     # ✅ Jedini prozor s pozitivnim ROI
        'min_snapshots': 3,          # Minimum podataka za pouzdanost
        'min_confidence': 0.80,      # NOVO: <80% gubi -17% do -43%
    },
}
